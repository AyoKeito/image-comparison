...
from exceptions import ImageLoadError, CacheError
...
class ImageProcessor:
    ...
    def load_and_cache(self, image_path: Path, max_width: int, max_height: int) -> QPixmap:
        ...
        try:
            pixmap = self._load_and_process_image(image_path, max_width, max_height)
    ...
    def _load_and_process_image(self, image_path: Path, max_width: int, max_height: int) -> QPixmap:
        ...
        try:
            ...
            raise ImageLoadError(str(image_path), f"Failed to open image file", e)
        except Exception as e:
            raise ImageLoadError(str(image_path), f"Failed to process image", e)
    ...
    def _pil_to_qimage(self, pil_image: Image.Image) -> QImage:
        ...
        # Convert to RGBA for consistent format
